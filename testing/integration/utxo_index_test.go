...
func TestUTXOIndex(t *testing.T) {
	// Setup a single nonsensed instance
	...
	err := nonsensed.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := nonsensed.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedSompi := uint64(blockAmountToMine * constants.SompiPerNonsense * 50)
	getBlockCountResponse, err := nonsensed.rpcClient.GetBlockCount()
	if err != nil {
		t.Fatalf("Error Retriving BlockCount: %s", err)
	}
	rewardsMinedViaBlockCountSompi := uint64(
		(getBlockCountResponse.BlockCount - 2) * constants.SompiPerNonsense * 50, // -2 because of genesis and virtual.
	)
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = nonsensed.rpcClient.SubmitTransaction(rpcTransaction, transactionID, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := nonsensed.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) (*appmessage.RPCTransaction, string) {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixNonsenseSim)
	if err != nil {
