...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of nonsensed instances
	nonsensed1, nonsensed2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := nonsensed1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In nonsensed2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, nonsensed2)
	}

	// Connect the two nonsenseds. This should trigger sync
	// between the two nodes
	connect(t, nonsensed1, nonsensed2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := nonsensed1.rpcClient.GetVirtualSelectedParentChainFromBlock(
