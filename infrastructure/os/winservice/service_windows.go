...
// long-running nonsensedMain (which is the real meat of nonsensed), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start nonsensedMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once nonsensed is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("nonsensedMain-windows", func() {
...
// logServiceStart logs information about nonsensed when the main server has
