...
NonsensedMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a NonsensedMessage) the
RPC server will respond with a ResponseMessage (likewise wrapped in a
NonsensedMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network nonsensed is
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which nonsense address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether nonsensed thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when nonsensed is not synced. That is because when nonsensed isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known nonsensed
...
p2p peers currently connected to this nonsensed.
...
| isOutbound | [bool](#bool) |  | Whether this nonsensed initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this nonsensed |
...
AddPeerRequestMessage adds a peer to nonsensed&#39;s outgoing
connection list. This will, in most cases, result in nonsensed
...
lowHash up to this nonsensed&#39;s current virtual.
...
this nonsensed. Note that this number may decrease as pruning occurs.
...
current state of this nonsensed&#39;s DAG.
...
ShutDownRequestMessage shuts down this nonsensed.
...
This call is only available when this nonsensed was started with
`--utxoindex`.
