...
const (
	defaultConfigFilename      = "nonsensed.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "nonsensed.log"
	defaultErrLogFilename      = "nonsensed_err.log"
	...
	sampleConfigFilename    = "sample-nonsensed.conf"
...
var (
	// DefaultAppDir is the default home directory for nonsensed.
	DefaultAppDir = util.AppDir("nonsensed", false)
...
//go:embed sample-nonsensed.conf
...
// Flags defines the configuration options for nonsensed.
//
...
type Flags struct {
	...
	Listeners                       []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 39111, testnet: 39211)"`
	...
	RPCListeners                    []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 39110, testnet: 39210)"`
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in NNN/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for nonsensed.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default nonsensed configuration
...
//
// The above results in nonsensed functioning properly without any config settings
...
// applyDefaultMainnetPeers gives a fresh mainnet node deterministic bootstrap
// peers without overriding an operator's explicit --addpeer or --connect list.
func applyDefaultMainnetPeers(cfg *Config) {
	if cfg.NetParams() != &dagconfig.MainnetParams || len(cfg.AddPeers) != 0 || len(cfg.ConnectPeers) != 0 {
		return
	}
	cfg.AddPeers = append([]string(nil), mainnetBootstrapPeers...)
}

// createDefaultConfig copies the file sample-nonsensed.conf to the given destination path,
