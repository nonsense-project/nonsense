...
import (
	...
	"github.com/nonsense-project/nonsense/v2/cmd/nonsenseminer/templatemanager"
...
func mineLoop(client *minerClient, numberOfBlocks uint64, targetBlocksPerSecond float64, mineWhenNotSynced bool,
	lightDataset bool, miningAddr util.Address) error {
	...
	spawn("templatesLoop", func() {
		templatesLoop(client, miningAddr, lightDataset, errChan)
	})
...
func getBlockForMining(mineWhenNotSynced bool) (*externalapi.DomainBlock, *pow.State) {
	...
	for {
		...
		if !isSynced && !mineWhenNotSynced {
			if shouldLog {
				log.Warnf("Nonsensed is not synced. Skipping current block template")
			}
...
func templatesLoop(client *minerClient, miningAddr util.Address, lightDataset bool, errChan chan error) {
	getBlockTemplate := func() {
		template, err := client.GetBlockTemplate(miningAddr.String(), "nonsenseminer-"+version.Version())
		if nativeerrors.Is(err, router.ErrTimeout) {
		...
		err = templatemanager.Set(template, backendLog, !lightDataset)
