package libnonsensewallet_test
...
func createUnsignedTransactionSerialized(
	...
	payments []*libnonsensewallet.Payment,
	selectedUTXOs []*libnonsensewallet.UTXO) ([]byte, error) {

	tx, err := libnonsensewallet.CreateUnsignedTransaction(extendedPublicKeys, minimumSignatures, payments, selectedUTXOs)
	if err != nil {
		return nil, err
	}
	...
	testutils.ForAllPaths(t, func(t *testing.T, version uint32) {
		testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
			...
			forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
				...
				path := "m/1/2/3"
				address, err := libnonsensewallet.Address(params, publicKeys, minimumSignatures, path, ecdsa)
				if err != nil {
				...
				isFullySigned, err := libnonsensewallet.IsTransactionFullySigned(unsignedTransaction)
				if err != nil {
				...
				_, err = libnonsensewallet.ExtractTransaction(unsignedTransaction, ecdsa)
				...
				signedTxStep1, err := libnonsensewallet.Sign(params, mnemonics[:1], unsignedTransaction, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				isFullySigned, err = libnonsensewallet.IsTransactionFullySigned(signedTxStep1)
				if err != nil {
					t.Fatalf("IsTransactionFullySigned: %+v", err)
				}
				...
				signedTxStep2, err := libnonsensewallet.Sign(params, mnemonics[1:2], signedTxStep1, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				extractedSignedTxStep2, err := libnonsensewallet.ExtractTransaction(signedTxStep2, ecdsa)
				if err != nil {
					t.Fatalf("ExtractTransaction: %+v", err)
				}

				signedTxOneStep, err := libnonsensewallet.Sign(params, mnemonics[:2], unsignedTransaction, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				extractedSignedTxOneStep, err := libnonsensewallet.ExtractTransaction(signedTxOneStep, ecdsa)
				if err != nil {
					t.Fatalf("ExtractTransaction: %+v", err)
				}
		...
		testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
			...
			forSchnorrAndECDSA(t, func(t *testing.T, ecdsa bool) {
				...
				for i := 0; i < numKeys; i++ {
					...
					publicKeys[i], err = libnonsensewallet.MasterPublicKeyFromMnemonic(&consensusConfig.Params, mnemonics[i], false, version)
					if err != nil {
						t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
					}
				}
				...
				signedTx, err := libnonsensewallet.Sign(params, mnemonics, unsignedTransaction, ecdsa, version)
				if err != nil {
					t.Fatalf("Sign: %+v", err)
				}

				tx, err := libnonsensewallet.ExtractTransaction(signedTx, ecdsa)
				if err != nil {
					t.Fatalf("ExtractTransaction: %+v", err)
				}
...
func TestMaxSompi(t *testing.T) {
	testutils.ForAllPaths(t, func(t *testing.T, version uint32) {
		testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
			...
			cfg.PreDeflationaryPhaseBaseSubsidy = 20e6 * constants.SompiPerNonsense
			...
			for i := 0; i < numKeys; i++ {
				...
				publicKeys[i], err = libnonsensewallet.MasterPublicKeyFromMnemonic(&cfg.Params, mnemonics[i], false, version)
				if err != nil {
					t.Fatalf("MasterPublicKeyFromMnemonic: %+v", err)
				}
			}
			...
			selectedUTXOsForTxWithLargeInputAmount := []*libnonsensewallet.UTXO{
				{
					Outpoint: &externalapi.DomainOutpoint{
			...
			signedTxWithLargeInputAmount, err := libnonsensewallet.Sign(params, mnemonics, unsignedTxWithLargeInputAmount, false, version)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			txWithLargeInputAmount, err := libnonsensewallet.ExtractTransaction(signedTxWithLargeInputAmount, false)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
			...
			selectedUTXOsForTxWithLargeInputAndOutputAmount := []*libnonsensewallet.UTXO{
				{
					Outpoint: &externalapi.DomainOutpoint{
			...
			unsignedTxWithLargeInputAndOutputAmount, err := createUnsignedTransactionSerialized(publicKeys, minimumSignatures,
				[]*libnonsensewallet.Payment{{
					Address: address,
					Amount:  22e6 * constants.SompiPerNonsense,
				}}, selectedUTXOsForTxWithLargeInputAndOutputAmount)
			if err != nil {
				t.Fatalf("CreateUnsignedTransactions: %+v", err)
			}

			signedTxWithLargeInputAndOutputAmount, err := libnonsensewallet.Sign(params, mnemonics, unsignedTxWithLargeInputAndOutputAmount, false, version)
			if err != nil {
				t.Fatalf("Sign: %+v", err)
			}

			txWithLargeInputAndOutputAmount, err := libnonsensewallet.ExtractTransaction(signedTxWithLargeInputAndOutputAmount, false)
			if err != nil {
				t.Fatalf("ExtractTransaction: %+v", err)
			}
