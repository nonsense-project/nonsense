...
// Additionally, every transaction with send value > ~0.1 NNN should succeed (at most ~99K storage mass for payment
// output, thus overall lower than standard mass upper bound which is 100K gram)
const minChangeTarget = constants.SompiPerNonsense * 10
...
func (s *server) calculateFeeLimits(requestFeePolicy *pb.FeePolicy) (feeRate float64, maxFee uint64, err error) {
	...
	case nil:
		...
		feeRate = estimate.Estimate.NormalBuckets[0].Feerate
		// Default to a bound of max 1 NNN as fee
		maxFee = constants.SompiPerNonsense
	}
...
func (s *server) createUnsignedTransactions(address string, amount uint64, isSendAll bool, fromAddressesString []string, useExistingChangeAddress bool, requestFeePolicy *pb.FeePolicy) ([][]byte, error) {
	...
	if changeSompi > 0 {
		payments = append(payments, &libnonsensewallet.Payment{
			Address: changeAddress,
			Amount:  changeSompi,
		})
	}
	unsignedTransaction, err := libnonsensewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) selectUTXOs(spendAmount uint64, isSendAll bool, feeRate float64, maxFee uint64, fromAddresses []*walletAddress) (
	selectedUTXOs []*libnonsensewallet.UTXO, totalReceived uint64, changeSompi uint64, err error) {
...
func (s *server) selectUTXOsWithPreselected(preSelectedUTXOs []*walletUTXO, allowUsed map[externalapi.DomainOutpoint]struct{}, spendAmount uint64, isSendAll bool, feeRate float64, maxFee uint64, fromAddresses []*walletAddress) (
	...
	iteration := func(utxo *walletUTXO, avoidPreselected bool) (bool, error) {
		...
		selectedUTXOs = append(selectedUTXOs, &libnonsensewallet.UTXO{
	...
	if totalValue < totalSpend {
		return nil, 0, 0, errors.Errorf("Insufficient funds for send: %f required, while only %f available",
			float64(totalSpend)/constants.SompiPerNonsense, float64(totalValue)/constants.SompiPerNonsense)
	}

	return selectedUTXOs, totalReceived, totalValue - totalSpend, nil
}

func (s *server) estimateFee(selectedUTXOs []*libnonsensewallet.UTXO, feeRate float64, maxFee uint64, recipientValue uint64) (uint64, error) {
	...
	var mockPayments []*libnonsensewallet.Payment
	if totalValue > recipientValue {
		mockPayments = []*libnonsensewallet.Payment{
			{
				Address: fakeAddr,
	...
	mockTx, err := libnonsensewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
...
func (s *server) estimateFeePerInput(feeRate float64) (uint64, error) {
	mockUTXO := &libnonsensewallet.UTXO{
	...
	mockTx, err := libnonsensewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, []*libnonsensewallet.UTXO{mockUTXO})
	if err != nil {
		return 0, err
	}
	...
	mockTxWithoutUTXO, err := libnonsensewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, nil)
	if err != nil {
		return 0, err
