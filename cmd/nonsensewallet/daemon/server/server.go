...
import (
	...
	"github.com/nonsense-project/nonsense/v2/infrastructure/network/rpcclient"
...
type server struct {
	pb.UnimplementedNonsensewalletdServer
...
// Start starts the nonsensewalletd server
func Start(params *dagconfig.Params, listen, rpcServer string, keysFilePath string, profile string, timeout uint32) error {
	...
	if dagInfo.NetworkName == "nonsense-testnet-1" {
	...
	if keysFile.Version == 1 {
		...
		log.Infof("new one and transfer NNN to it.")
		log.Infof("---")
	}
	...
	pb.RegisterNonsensewalletdServer(grpcServer, serverInstance)
