...
import (
	...
	"github.com/nonsense-project/nonsense/v2/domain/miningmanager/mempool"
...
func (s *server) mergeTransaction(
	...
	utxos := make([]*libnonsensewallet.UTXO, len(splitTransactions))
	for i, splitTransaction := range splitTransactions {
		output := splitTransaction.Tx.Outputs[0]
		utxos[i] = &libnonsensewallet.UTXO{
	...
	return libnonsensewallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) splitAndInputPerSplitCounts(transaction *serialization.PartiallySignedTransaction, transactionMass uint64,
	...
	// Since the transaction was generated by nonsensewallet, we assume all inputs have the same number of signatures, and
	// thus - the same mass.
...
func (s *server) createSplitTransaction(transaction *serialization.PartiallySignedTransaction,
	...
	selectedUTXOs := make([]*libnonsensewallet.UTXO, 0, endIndex-startIndex)
	totalSompi := uint64(0)
...
func createTransactionWithJunkFieldsForMassCalculation(transaction *serialization.PartiallySignedTransaction, ecdsa bool, minimumSignatures uint32, txMassCalculator *txmass.Calculator) (*externalapi.DomainTransaction, error) {
	...
	return libnonsensewallet.ExtractTransactionDeserialized(transaction, ecdsa)
}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libnonsensewallet.UTXO, requiredAmount uint64, feeRate float64) (
	additionalUTXOs []*libnonsensewallet.UTXO, totalValueAdded uint64, err error) {
	...
	for _, utxo := range s.utxosSortedByAmount {
		...
		additionalUTXOs = append(additionalUTXOs, &libnonsensewallet.UTXO{
