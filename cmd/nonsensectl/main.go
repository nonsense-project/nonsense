...
import (
	...
	"github.com/nonsense-project/nonsense/v2/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		nonsensedMessage, err := client.Post(&protowire.NonsensedMessage{Payload: &protowire.NonsensedMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := nonsensedMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	nonsensedMessage := &protowire.NonsensedMessage{}
	err := protojson.Unmarshal([]byte(response), nonsensedMessage)
	if err != nil {
	...
	return marshalOptions.Format(nonsensedMessage)
}
