...
// 2. A wrapper that implements isNonsensedMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `NonsensedMessage_` prefix and `Request` suffix
...
// generateNonsensedMessage generates a wrapped NonsensedMessage with the given `commandValue`
func generateNonsensedMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.NonsensedMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	nonsensedMessage := reflect.New(reflect.TypeOf(protowire.NonsensedMessage{}))
	nonsensedMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return nonsensedMessage.Interface().(*protowire.NonsensedMessage), nil
}
