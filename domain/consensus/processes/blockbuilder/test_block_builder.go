...
func (bb *testBlockBuilder) buildBlockWithParents(stagingArea *model.StagingArea, parentHashes []*externalapi.DomainHash,
	...
	*externalapi.DomainBlock, externalapi.UTXODiff, error) {
	...
	// Keep the test builder consistent with the production builder during the
	// post-hardfork difficulty bootstrap window. Nonsense activates V2 at
	// launch, so tests reach this branch immediately.
	if daaScore <= bb.hfDAAScore+uint64(bb.difficultyManager.DifficultyAdjustmentWindowSize()) &&
		daaScore >= bb.hfDAAScore {
