...
// Outpoint defines a nonsense data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new nonsense transaction outpoint point with the
...
// TxIn defines a nonsense transaction input.
type TxIn struct {
...
// NewTxIn returns a new nonsense transaction input with the provided
...
// TxOut defines a nonsense transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new nonsense transaction output with the provided
...
// MsgTx implements the Message interface and represents a nonsense tx message.
...
type MsgTx struct {
	...
	Gas          uint64
	Payload      []byte
	// MassCommitment is consensus data. It must survive both P2P and database
	// serialization because TransactionHash commits to a non-zero value.
