...
import (
	...
	"github.com/nonsense-project/nonsense/v2/domain/consensus/utils/pow"
	...
	"github.com/nonsense-project/nonsense/v2/infrastructure/os/execenv"
	"github.com/nonsense-project/nonsense/v2/infrastructure/os/limits"
	"github.com/nonsense-project/nonsense/v2/infrastructure/os/signal"
	"github.com/nonsense-project/nonsense/v2/infrastructure/os/winservice"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "nonsensedsvc",
	DisplayName: "Nonsensed Service",
	Description: "Downloads and stays synchronized with the Nonsense blockDAG and " +
		"provides DAG services to applications.",
}

type nonsensedApp struct {
	cfg *config.Config
}

// StartApp starts the nonsensed app, and blocks until it finishes running
func StartApp() error {
...
func (app *nonsensedApp) main(startedChan chan<- struct{}) error {
	...
	log.Infof("Using NonsenseHashV2 impl: %s", pow.GetHashingAlgoVersion())
	...
	if err != nil {
		log.Errorf("Unable to start nonsensed: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down nonsensed...")
		...
		log.Infof("Nonsensed shutdown complete")
	}()
