...
var legacyMassCommitments = map[string]uint64{
...
type Calculator struct {
	...
	// The parameter for scaling inverse NNN value to mass units (KIP-0009)
...
func NewCalculator(massPerTxByte, massPerScriptPubKeyByte, massPerSigOp uint64) *Calculator {
	return &Calculator{
		...
		storageMassParameter:    constants.SompiPerNonsense * 10_000,
	}
}

// PopulateMissingMassCommitment restores commitments for the three mainnet
// transactions mined by the first Nonsense daemon build. That build committed
// the wallet-provided overall mass to the transaction hash, but omitted the
// field from its database and P2P protobuf encodings. The exact commitments
// were recovered from the corresponding block Merkle roots. Unknown
// transactions must remain untouched: their storage mass cannot be recovered
// without the spent UTXO entries.
func PopulateMissingMassCommitment(transaction *externalapi.DomainTransaction) {
	if transaction.MassCommitment != 0 || transactionhelper.IsCoinBase(transaction) {
		return
	}
	if mass, ok := legacyMassCommitments[consensushashing.TransactionID(transaction).String()]; ok {
		transaction.MassCommitment = mass
	}
}
