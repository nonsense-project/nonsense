...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("nonsensed-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("nonsensed-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("NONSENSED-SYNCER",
		"nonsensed",
	...
	syncedCmd, err := common.StartCmd("NONSENSED-SYNCED",
		"nonsensed",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
