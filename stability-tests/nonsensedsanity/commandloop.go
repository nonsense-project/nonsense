...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("nonsensedsanity-nonsensed-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("nonsensed", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "NONSENSED-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "NONSENSED-STDERR")
