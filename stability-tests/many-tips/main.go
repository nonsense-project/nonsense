...
import (
	...
	"github.com/nonsense-project/nonsense/v2/domain/consensus/utils/mining"
	"github.com/nonsense-project/nonsense/v2/util"
	"github.com/kaspanet/go-secp256k1"

	"github.com/nonsense-project/nonsense/v2/stability-tests/common"
	"github.com/nonsense-project/nonsense/v2/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("nonsensed-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("nonsensed datadir: %s", dataDir)

	nonsensedCmd, err := common.StartCmd("NONSENSED",
		"nonsensed",
	...
	spawn("startNode-nonsensedCmd.Wait", func() {
		err := nonsensedCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("nonsensedCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes nonsensed closes ungracefully
				log.Errorf("nonsensedCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(nonsensedCmd, "nonsensedCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	nonsenseminerCmd, err := common.StartCmd("MINER",
		"nonsenseminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("nonsense-miner-Cmd.Wait", func() {
		err := nonsenseminerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(nonsenseminerCmd, "nonsenseminerCmd")
	return nil
}
